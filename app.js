...
regForm.addEventListener('submit', function (e) {
	...
	if(usernameReg.value.length == 0 || passwordReg.value.length < 8) {
		alert("password should not be less than 8 Characters");
	}
	else{
		if (passwordReg.value.length < 8, /^\d+$/.test(passwordReg.value), !/[a-z]/.test(passwordReg.value) || !/[A-Z]/.test(passwordReg.value)) {
			alert("Password should not be less than 8 Characters and must contain both uppercase and lowercase characters, and not consist of only integers")
		} else {
			// Store the username and passwords inside the JavaScript Object 
			usernameAndPasswords[usernameReg.value] = passwordReg.value;
			console.log(usernameAndPasswords);
